...
fn parse_html_input(input: ParseStream<'_>) -> syn::Result<HtmlInput> {
...
pub fn html_impl(input: TokenStream) -> TokenStream {
    let render_input: HtmlInput = match syn::parse2(input) {
        Ok(a) => a,
    ...
    let formatter = Ident::new("__plait_html_formatter", render_input.span);

    for node in render_input.nodes {
        push_statements_for_node(&mut statements, &formatter, node);
    }

    quote! {
        ::plait::RenderFn::new(move |#formatter : &mut ::plait::HtmlFormatter| {
            #(#statements)*
        })
    }
}
    ...
    for attribute in element.attributes {
        match attribute {
            Attribute::NameValue { name, value } => {
                ...
                match value {
                    ...
                    Some(AttributeValue::Optional { expr }) => {
                        ...
                        statements.push(quote! {
                            #formatter .write_optional_attribute( #name, ::core::option::Option::as_ref(&(#expr)), #escape_mode ).unwrap();
                        });
                    },
            ...
            Attribute::Spread { expr } => statements.push(quote! {
                #formatter .spread_attributes( ::core::clone::Clone::clone(&(#expr)) ).unwrap();
            }),
        }
    }
