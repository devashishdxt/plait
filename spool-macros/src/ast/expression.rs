...
/// A dynamic expression that can be evaluated at runtime.
///
...
fn parse_expression(input: ParseStream<'_>) -> syn::Result<Expression> {
    let content;
    ...
    Ok(Expression { expr, escape_mode })
}
...
mod tests {
    ...
    fn test_parse_expression_without_escape_mode() {
        let input = quote! { (1 + 2) };
        let expression: Expression = syn::parse2(input).unwrap();
    ...
    fn test_parse_expression_with_escape_mode_invalid() {
        ...
        let result: syn::Result<Expression> = syn::parse2(input);
